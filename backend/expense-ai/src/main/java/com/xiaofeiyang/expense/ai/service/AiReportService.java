...
public class AiReportService {
    ...
    private String[] buildPrompts(int year, int month, Long userId) {
        MonthlyStatsDTO current = statisticsClient.getMonthlyStats(new MonthlyStatsRequest(year, month));
        MonthlyStatsDTO previous = null;
        if (month == 1) {
            try { previous = statisticsClient.getMonthlyStats(new MonthlyStatsRequest(year - 1, 12)); }
            catch (Exception ignored) {}
        } else {
            try { previous = statisticsClient.getMonthlyStats(new MonthlyStatsRequest(year, month - 1)); }
            catch (Exception ignored) {}
        }
    ...
    private String formatStats(MonthlyStatsDTO stats) {
